...
for i = 1:nFA
    ...
    for j = 1:nCh
        A = amb(x(ch(j),:)', 512, 2*length(sampleInd));
        A = downsample(downsample(A,64)',32)';
        XFA(i,(numel(A)*(j-1))+(1:numel(A))) = real(A(:))';
    end
end
for i = 1:nLM
    sampleInd = 769:1024;
    ...
    for j = 1:nCh
        A = amb(x(ch(j),:)', 512, 2*length(sampleInd));
        A = downsample(downsample(A,64)',32)';
        XLM(i,(numel(A)*(j-1))+(1:numel(A))) = real(A(:))';
    end
end
for i = 1:nOB
    sampleInd = 769:1024;
    ...
    for j = 1:nCh
        A = amb(x(ch(j),:)', 512, 2*length(sampleInd));
        A = downsample(downsample(A,64)',32)';
        XOB(i,(numel(A)*(j-1))+(1:numel(A))) = real(A(:))';
    end
end
