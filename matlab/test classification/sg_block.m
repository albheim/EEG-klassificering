...
for i = 1:nFA
    ...
    for j = 1:nCh
        W = mtspectrogram(x(ch(j),:)', L, 512, 2*length(sampleInd),nstep);
        W = aux_bsum(W,block_nrows,block_ncols);
        XFA(i,n_feat*(j-1)+(1:n_feat)) = W(:);
    end
end
for i = 1:nLM
    sampleInd = 769:1024;
